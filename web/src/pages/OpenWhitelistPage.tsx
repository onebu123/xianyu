...
export function OpenWhitelistPage() {
  ...
  const loader = useCallback(async () => {
    return apiRequest<WorkspaceOverviewResponse>('/api/workspaces/open-whitelist', undefined);
  }, []);
  ...
  const columns = useMemo<TableProps['columns']>(
    () => [
      { title: '规则', dataIndex: 'title', width: 200 },
      { title: '状态', dataIndex: 'status', width: 90, render: (v: string) => <Tag color={v === 'done' ? 'success' : v === 'active' ? 'processing' : 'default'}>{v === 'done' ? '已生效' : v === 'active' ? '审核中' : '待配置'}</Tag> },
  ...
  return (
    <PageContainer title="白名单管理" subTitle="管理调用来源、IP 白名单和权限生效范围。" style={{ paddingInline: 0 }}
      extra={[<Button key="refresh" icon={<ReloadOutlined />} onClick={() => void reload()}>刷新</Button>]}
    >
      <div className="page-grid">
        <Alert type="info" showIcon icon={<SafetyOutlined />} message="白名单说明" description="通过 IP 白名单和来源限制，保护开放平台接口的安全访问。" />
        {error && <Alert type="error" showIcon message={error} />}
        {loading || !data ? <Skeleton active paragraph={{ rows: 6 }} /> : (
          <>
            {summary.length > 0 && <SummaryCards items={summary} />}
            <div className="glass-panel" style={{ padding: 24 }}>
              <Typography.Title level={4} style={{ marginBottom: 16 }}>白名单规则</Typography.Title>
