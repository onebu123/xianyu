...
export function OpenSettingsPage() {
  ...
  const loader = useCallback(async () => {
    return apiRequest<WorkspaceOverviewResponse>('/api/workspaces/open-settings', undefined);
  }, []);
  ...
  const columns = useMemo<TableProps['columns']>(
    () => [
      { title: '配置项', dataIndex: 'title', width: 200 },
      { title: '状态', dataIndex: 'status', width: 90, render: (v: string) => <Tag color={v === 'done' ? 'success' : v === 'active' ? 'processing' : 'default'}>{v === 'done' ? '已配置' : v === 'active' ? '配置中' : '待配置'}</Tag> },
  ...
  return (
    <PageContainer title="开放平台设置" subTitle="配置密钥、回调地址与调用白名单。" style={{ paddingInline: 0 }}
      extra={[<Button key="refresh" icon={<ReloadOutlined />} onClick={() => void reload()}>刷新</Button>]}
    >
      <div className="page-grid">
        <Alert type="info" showIcon icon={<SettingOutlined />} message="设置说明" description="在此配置开放平台的接入密钥、Webhook 回调地址和安全策略。" />
        {error && <Alert type="error" showIcon message={error} />}
        {loading || !data ? <Skeleton active paragraph={{ rows: 6 }} /> : (
          <>
            {summary.length > 0 && <SummaryCards items={summary} />}
            <div className="glass-panel" style={{ padding: 24 }}>
              <Typography.Title level={4} style={{ marginBottom: 16 }}>配置清单</Typography.Title>
